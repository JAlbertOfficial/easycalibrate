    ...
    if 'df' in st.session_state and 'model' in st.session_state:
        ...
        if bp_significant or sw_significant:
            st.markdown("""
                ...
                The analysis of relative errors has shown that the precision in the lower range of the calibration is negatively affected. 
                It is strongly recommended to try the improved calibration model.
                </div>
            """, unsafe_allow_html=True)
        else:
            st.markdown("""
                ...
                The analysis of relative errors has shown that there is no significant negative effect on the precision in the lower range of the calibration.
...
def render_basic_calibration():
    if st.session_state.get('data_imported'):
        if st.session_state.get('model_trained'):
            ...
            bc_section = st.sidebar.radio(
                "",
                ["Import Data", "Basic Calibration Model", "Model Evaluation"],
                index=["Import Data", "Basic Calibration Model", "Model Evaluation"].index(st.session_state.get('current_section', "Import Data"))
            )
        else:
            bc_section = st.sidebar.radio(
    ...
    elif bc_section == "Model Evaluation" and st.session_state.get('model_trained'):
        bc_model_evaluation()
